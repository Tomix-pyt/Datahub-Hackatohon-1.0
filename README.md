...
```text
├── cortex/
│   ├── graph.py             # LangGraph control flow & node logic
│   ├── models.py            # Typed Pydantic data structures
│   ├── memory_episodic.py   # ChromaDB vector experience store
│   ├── memory_semantic.py   # DataHub SDK & custom aspect writer
│   ├── diff.py              # Snapshot & lineage comparison engine
│   ├── reflection.py        # Recurrence guard & promotion gate
│   ├── llm.py               # LLM interface (Groq + mock engine)
│   └── procedure.py         # YAML runbook loader
├── procedures/
│   ├── schema_drift.yaml    # Schema change triage runbook
│   ├── freshness.yaml       # SLA freshness violation runbook
│   └── default.yaml         # Fallback triage procedure
├── scripts/
│   └── simulate_incident.py # Incident & schema drift simulator
├── data/chroma              # ChromaDB local storage
├── app.py                   # Offline CLI demo runner
├── app_live.py              # Live DataHub & Groq integration runner
